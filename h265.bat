...
for /d %%a in (*) do (
    ...
    if /i not "%%a"=="output" (
        ...
        for %%f in ("%%a\*.avi") do (
            ...
            REM Execute FFmpeg conversion using NVENC for H.265 (HEVC)
            ffmpeg -y -i "%%f" -c:v hevc_nvenc -preset fast -b:v 5M -c:a copy "%output_root%\%%a\!filename!.mp4" 2>> conversion_errors.log
        )
    )
)
