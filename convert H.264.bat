...
set "root_dir=C:\Users\akash\Downloads\UCF101\UCF-101"
...
for /d %%a in (*) do (
    ...
    if /i not "%%a"=="output" (
        ...
        for %%f in ("%%a\*.avi") do (
            ...
            REM Execute FFmpeg conversion ensuring no path issues
            ffmpeg -y -i "%%f" -c:v libx264 -preset slow -crf 18 -c:a copy "%output_root%\%%a\!filename!.mp4" 2>> conversion_errors.log
        )
    )
)
